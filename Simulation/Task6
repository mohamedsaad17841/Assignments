...
int main ()
{
	...
	for(int i=0;i<timeOfRuns;i++){
		...
		CashierAvailTime = completion_Time_C[i];
		T_C[i] = completion_Time_C[i] - Arrival_Time[i];        ///cashier time
	}
	...
	double Arr_SOUP2 = 0.0;
	normal_distribution<> Arr_Soup_rand(1,0.25);
	...
	normal_distribution<> ST_Soup(1,0.25);
	...
	cout<<"\n if getting salad only is:  " << Avg_T_C + Avg_T_Salad  <<endl;
	cout<<"\n if getting both is:  " << Avg_T_C + Avg_T_Salad  + Avg_T_Soup <<endl;
